...
)
{
	...
	else
	{
		...
		soltestAssert(yulStack.parserResult()->dialect());
	}
	return yulStack;
}
...
yul::Block yul::test::disambiguate(std::string const& _source)
{
	...
	Disambiguator disambiguator(*yulStack.parserResult()->dialect(), *yulStack.parserResult()->analysisInfo, {});
	Statement disambiguatedAST = disambiguator(yulStack.parserResult()->code()->root());
	soltestAssert(std::holds_alternative<Block>(disambiguatedAST));
	return std::get<Block>(std::move(disambiguatedAST));
}
